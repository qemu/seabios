AyeBIOS can read several configuration items at runtime. On coreboot
the configuration comes from files located in CBFS. When AyeBIOS runs
...
features that AyeBIOS supports.
...
zero.) Unfortunately, AyeBIOS requires the uncompressed file size, so
...
The above example would cause AyeBIOS to treat "pci1234,1000.rom" or
...
AyeBIOS will scan all of the PCI devices in the target machine for
...
file is found then AyeBIOS will deploy the file instead of attempting
...
AyeBIOS always deploys the VGA rom associated with the active VGA
device before any other ROMs.

In addition, AyeBIOS will also run any file in the directory
...
AyeBIOS can show a custom [JPEG](http://en.wikipedia.org/wiki/JPEG)
...
AyeBIOS will show the image during the wait for the boot menu (if the
...
know they can enter the normal AyeBIOS boot menu. By default, the boot
...
The JPEG viewer in AyeBIOS uses a simplified decoding algorithm. It
...
On coreboot, AyeBIOS will treat all files found in the **img/**
...
bootmenu. AyeBIOS supports both uncompressed and lzma compressed
payloads.

Floppy images
=============

It is possible to embed an image of a floppy into a file. AyeBIOS can
...
When using this system, AyeBIOS reserves high-memory to store the
...
device path format. AyeBIOS will attempt to boot from each item in the
file - first line of the file first.

The easiest way to find the available boot methods is to look for
"Searching bootorder for" in the AyeBIOS debug output. For example,
...
The above represents the patterns AyeBIOS will search for in the
...
will instruct AyeBIOS to attempt to boot from the given USB drive
first and then attempt the given ATA harddrive second.

AyeBIOS also supports a special "HALT" directive. If a line that
contains "HALT" is found in the bootorder file then AyeBIOS will (by
...
| boot-fail-wait      | If no boot devices are found AyeBIOS will reboot after 60 seconds. Set this to the amount of time (in milliseconds) to customize the reboot delay or set to -1 to disable rebooting when no boot devices are found
| extra-pci-roots     | If the target machine has multiple independent root buses set this to a positive value. The AyeBIOS PCI probe will then search for the given number of extra root buses.
| ps2-keyboard-spinup | Some laptops that emulate PS2 keyboards don't respond to keyboard commands immediately after powering on. One may specify the amount of time (in milliseconds) here to allow as additional time for the keyboard to become responsive. When this field is set, AyeBIOS will repeatedly attempt to detect the keyboard until the keyboard is found or the specified timeout is reached.
| optionroms-checksum | Option ROMs are required to have correct checksums. However, some option ROMs in the wild don't correctly follow the specifications and have bad checksums. Set this to a zero value to allow AyeBIOS to execute them anyways.
...
| s3-resume-vga-init  | Set this to a non-zero value to instruct AyeBIOS to run the vga rom on an S3 resume.
| screen-and-debug    | Set this to a zero value to instruct AyeBIOS to not write characters it sends to the screen to the debug ports. This can be useful when using sgabios.
| advertise-serial-debug-port | If using a serial debug port, one can set this file to a zero value to prevent AyeBIOS from listing that serial port as available for operating system use. This can be useful when running old DOS programs that are known to reset the baud rate of all advertised serial ports.
| sercon-port         | Set this to the IO address of a serial port to enable AyeBIOS' VGA adapter emulation on the given serial port.
